<?php
    ...
    $sql = "SELECT title, release_date, genres.genre AS genre, ratings.rating AS rating
            FROM dvd_titles
            JOIN genres
    ...
    if( isset($_GET['genre']) && !empty($_GET['genre']) ) {
    	$sql = $sql . " AND dvd_titles.genre_id LIKE '%" . $_GET['genre'] . "%'";
    }
    if( isset($_GET['rating']) && !empty($_GET['rating']) ) {
    	$sql = $sql . " AND dvd_titles.rating_id LIKE '%" . $_GET['rating'] . "%'";
    }
    if( isset($_GET['label']) && !empty($_GET['label']) ) {
    	$sql = $sql . " AND dvd_titles.label_id LIKE '%" . $_GET['label'] . "%'";
    }
    if( isset($_GET['format']) && !empty($_GET['format']) ) {
    	$sql = $sql . " AND dvd_titles.format_id LIKE '%" . $_GET['format'] . "%'";
    }
    if( isset($_GET['sound']) && !empty($_GET['sound']) ) {
    	$sql = $sql . " AND dvd_titles.sound_id LIKE '%" . $_GET['sound'] . "%'";
    }
    if( isset($_GET['format']) && !empty($_GET['format']) ) {
    	$sql = $sql . " AND dvd_titles.format_id LIKE '%" . $_GET['format'] . "%'";
    }
